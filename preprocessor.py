...
def preprocess(data):
    ...
    if re.search(r'\d{1,2}:\d{2}\s*(?:am|pm)', data, re.IGNORECASE):
        pattern = pattern_12hr  # Use 12-hour format pattern
        am_pm_format = True
    else:
        pattern = pattern_24hr  # Use 24-hour format pattern
        am_pm_format = False
    ...
    # Convert date column based on detected time format
    if am_pm_format:
